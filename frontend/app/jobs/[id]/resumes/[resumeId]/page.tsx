...
import { ChevronRight, FileText, Loader2, RefreshCcw } from "lucide-react";
...
export default function ResumeDetailPage() {
  ...
  const fetchResume = async () => {
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/query/get-resume?resume_id=${params.resumeId}`,
        {
          credentials: "include",
        },
      );
  ...
  const getSuggestions = (score: number) => {
    if (score >= 85) return "Excellent candidate - Highly recommended";
    if (score >= 70) return "Good candidate - Recommended";
    if (score >= 50) return "Average candidate - Considerable";
        ...
        <div className="mx-auto max-w-7xl">
          ...
          <div className="grid gap-8 lg:grid-cols-3">
            ...
            <div className="space-y-6 lg:col-span-2">
              ...
              <AcademicCard
                gpa={resume?.gpa ?? "N/A"}
                internships={resume?.num_internships ?? 0}
                schoolYear={resume?.school_year ?? "N/A"}
              />
              ...
              <Card>
                ...
                <CardContent>
                  <div className="space-y-4">
                    ...
                    <Progress
                      value={((resume?.gpa_contribution ?? 0) / 40) * 100}
                      className="h-2"
                    />
                  </div>
                  <div className="space-y-4">
                    ...
                    <Progress
                      value={
                        ((resume?.experience_contribution ?? 0) / 40) * 100
                      }
                      className="h-2"
                    />
                  </div>
                  <div className="space-y-4">
                    ...
                    <Progress
                      value={
                        ((resume?.impact_quality_contribution ?? 0) / 20) * 100
                      }
                      className="h-2"
                    />
                  </div>
